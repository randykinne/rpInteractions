...
import RandomPvP.Core.Util.Poll.Exception.PollException;
...
public class PollCmd extends RCommand implements Listener {
    ...
    public PollCmd() {
        ...
        setPlayerOnly(true);
        setMinimumArgs(1);
        setArgsUsage("stats <Poll>");
        setDescription("Used to see stats of a poll");
        ...
        if(ServerToggles.pollVotingEnabled()) {
            if (args[0].equalsIgnoreCase("create")) {
                if (pl.has(Rank.ADMIN)) {
                    ...
                    pl.message(MsgType.ERROR, "You need " + Rank.ADMIN.getFormattedName() + CC.GRY + CC.ITA + " to create polls.");
                }
                ...
                if (pl.has(Rank.ADMIN)) {
                    ...
                    pl.message(MsgType.ERROR, "You need " + Rank.ADMIN.getFormattedName() + CC.GRY + CC.ITA + " to edit polls.");
                }
                ...
                if(args.length == 2) {
                    PollManager manager = new PollManager();
                    if(manager.isValidPoll(args[1])) {
                        HashMap<String, Integer> data = manager.getData(args[1]);
                        pl.message(MsgType.INFO, "Showing poll data for poll " + CC.BLU + args[1].toUpperCase() + CC.GRY + "...");
                        pl.message("§8§l>§7 Option 1: " + data.get("option1") + " Votes");
                        pl.message("§8§l>§7 Option 2: " + data.get("option2") + " Votes");
                        pl.message("§8§l>§7 Option 3: " + data.get("option3") + " Votes");
                        pl.message("§8§l>§7 Option 4: " + data.get("option4") + " Votes");
                    } else {
                        pl.message(MsgType.ERROR, "You must supply a valid poll");
                    }
                } else {
